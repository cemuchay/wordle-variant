...
serve(async (req) => {
  ...
  const sendResendEmail = async (to: string, subject: string, html: string) => {
    ...
    if (!res.ok) {
      ...
      log(`Resend email delivery failed to ${to}. Status: ${res.status}. Error: ${errText}`);
    } else {
  ...
  const sendZohoEmail = async (to: string, subject: string, html: string) => {
    ...
    try {
      ...
      log(`Failed to send fallback email via Zoho SMTP to ${to}: ${(err as any).message}`);
      return false;
    }
  };
  ...
  const sendEmailWithFallback = async (to: string, subject: string, html: string) => {
    ...
    try {
      ...
      if (res) {
        log(`Resend failed with status ${res.status}. Attempting Zoho fallback...`);
      } else {
        log("Resend skipped. Attempting Zoho fallback...");
      }
    } catch (err) {
      log(`Resend failed with error: ${(err as any).message}. Attempting Zoho fallback...`);
    }
  ...
  try {
    ...
    if (action === "welcome-email") {
      ...
      log(`Sending welcome email to ${targetEmail} (Username: ${targetUsername})...`);
      
      const content = `
        <p>Welcome to <strong>Wordle Variant</strong>! We are thrilled to have you join our community of word puzzle enthusiasts. Whether you are a casual player or a competitive solver, here is everything you need to know to get started and dominate the leaderboards.</p>
        
        <div style="margin: 24px 0; background-color: #0b0f19; border: 1px solid #1f2937; border-radius: 16px; padding: 20px;">
          <h3 style="color: #6366f1; font-size: 16px; font-weight: 800; margin: 0 0 12px 0; text-transform: uppercase; letter-spacing: 0.05em; font-family: sans-serif;">🧩 How to Play</h3>
          <p style="margin-bottom: 12px; font-size: 14px; line-height: 1.6; color: #9ca3af;">You have <strong>6 attempts</strong> to guess a hidden 5-letter word. After each guess, the tiles change color to show how close your guess was:</p>
          ...
          <div style="text-align: center; margin: 20px 0;">
            ...
            <span style="display: block; font-size: 11px; color: #4b5563; margin-top: 8px;">Solving animation to discover the 5-letter word</span>
          </div>
        </div>

        <div style="margin: 24px 0; background-color: #0b0f19; border: 1px solid #1f2937; border-radius: 16px; padding: 20px;">
          <h3 style="color: #6366f1; font-size: 16px; font-weight: 800; margin: 0 0 12px 0; text-transform: uppercase; letter-spacing: 0.05em; font-family: sans-serif;">🔥 Challenge Mode</h3>
          <p style="margin: 0; font-size: 14px; line-height: 1.6; color: #9ca3af;">Go head-to-head against friends or other players in real-time! Create or join a challenge, solve the exact same target word, and see who can solve it faster and with fewer guesses.</p>
        </div>
        ...
        <div style="margin: 24px 0; background-color: #0b0f19; border: 1px solid #1f2937; border-radius: 16px; padding: 20px;">
          ...
          <p style="margin-bottom: 12px; font-size: 14px; line-height: 1.6; color: #9ca3af;">Climb both the daily and weekly global leaderboards based on your speed, attempts, and winning streaks!</p>
          <ul style="color: #9ca3af; font-size: 14px; padding-left: 20px; line-height: 1.6; margin: 0;">
            <li><strong>Speed & Guesses:</strong> Points are awarded based on how many guesses you take and how quickly you solve it.</li>
            <li><strong>Streaks:</strong> Keep your winning streak alive to earn streak bonuses. But watch out! You must play before midnight to save your streak.</li>
          </ul>
        </div>
      ...
      const html = getEmailHtml(targetUsername, targetUserId, "Welcome to Wordle Variant! 🧩", content);
      
      const success = await sendEmailWithFallback(targetEmail, "Welcome to Wordle Variant! 🧩", html);
      if (success) {
        sentCount++;
      ...
      await sendEmailWithFallback("cemuchay@gmail.com", `[Fellow Copy] [To: ${targetEmail}] Welcome to Wordle Variant! 🧩`, fellowHtml);
    ...
    if (action === "morning-reminders") {
      ...
      if (skippedRecipients && skippedRecipients.length > 0) {
        skippedRecipients.forEach((recipient: any) => {
          log(`Eligible skipped day: ${recipient.username} (${recipient.email})`);
        });
      }
      ...
      if (isMonday) {
        ...
        const { data, error: inactiveErr } = await supabase.rpc("get_three_day_inactive_recipients");
        if (inactiveErr) {
          log(`Error fetching 3-day inactive recipients: ${inactiveErr.message}`);
          throw inactiveErr;
        }
        inactiveRecipients = data || [];
        log(`Found ${inactiveRecipients.length} inactive recipients.`);
        inactiveRecipients.forEach((recipient: any) => {
          log(`Eligible inactive: ${recipient.username} (${recipient.email})`);
        });
      }
      ...
      if (skippedRecipients && skippedRecipients.length > 0) {
        for (const recipient of skippedRecipients) {
          ...
          log(`Attempting to send email to user ${recipient.username} (${recipient.email})...`);
          ...
          const fellowHtml = appendLogsToHtml(html, logs);
          log(`Sending fellow copy to cemuchay@gmail.com...`);
          await sendEmailWithFallback("cemuchay@gmail.com", `[Fellow Copy] [To: ${recipient.email}] Resume your play today! ⚡`, fellowHtml);
        }
      }
      ...
      if (inactiveRecipients.length > 0) {
        for (const recipient of inactiveRecipients) {
          ...
          log(`Attempting to send inactive reminder to user ${recipient.username} (${recipient.email})...`);
          ...
          const fellowHtml = appendLogsToHtml(html, logs);
          log(`Sending fellow copy to cemuchay@gmail.com...`);
          await sendEmailWithFallback("cemuchay@gmail.com", `[Fellow Copy] [To: ${recipient.email}] The Leaderboard Misses You! 🧩`, fellowHtml);
        }
      }
      ...
      if (sentCount === 0) {
        ...
        await sendEmailWithFallback("cemuchay@gmail.com", `[Logs] morning-reminders - No recipients eligible`, logsHtml);
      } else {
        log(`Finished morning-reminders processing. Total user emails sent: ${sentCount}`);
      }
    ...
    if (action === "evening-reminders") {
      ...
      if (recipients && recipients.length > 0) {
        recipients.forEach((recipient: any) => {
          log(`Eligible streak warning: ${recipient.username} (${recipient.email}, Current Streak: ${recipient.current_streak})`);
        });
        ...
        for (const recipient of recipients) {
          ...
          log(`Attempting to send streak warning email to user ${recipient.username} (${recipient.email})...`);
          const success = await sendEmailWithFallback(recipient.email, subject, html);
          if (success) {
            sentCount++;
            log(`Successfully sent email to ${recipient.email}`);
          } else {
          ...
          const fellowHtml = appendLogsToHtml(html, logs);
          log(`Sending fellow copy to cemuchay@gmail.com...`);
          await sendEmailWithFallback("cemuchay@gmail.com", `[Fellow Copy] [To: ${recipient.email}] ${subject}`, fellowHtml);
        }
      }
      ...
      if (sentCount === 0) {
        ...
        log(`Sending execution logs email to cemuchay@gmail.com...`);
        await sendEmailWithFallback("cemuchay@gmail.com", `[Logs] evening-reminders - No recipients eligible`, logsHtml);
      } else {
        log(`Finished evening-reminders processing. Total user emails sent: ${sentCount}`);
      }
    ...
    if (action === "weekly-report") {
      ...
      if (recipients && recipients.length > 0) {
        recipients.forEach((recipient: any) => {
          log(`Eligible weekly report recipient: ${recipient.username} (${recipient.email})`);
        });
      }
      ...
      if (recipients && recipients.length > 0 && leaderboard && leaderboard.length > 0) {
        for (const recipient of recipients) {
          ...
          const myIndex = leaderboard.findIndex((entry: any) => entry.username === recipient.username);
          ...
          relativeLeaderboard.forEach((entry: any) => {
            const globalIdx = leaderboard.findIndex((e: any) => e.username === entry.username);
          ...
          const subject = "Your Wordle Variant Weekly Report 📊";
          
          log(`Attempting to send weekly report email to user ${recipient.username} (${recipient.email})...`);
          const success = await sendEmailWithFallback(recipient.email, subject, html);
          if (success) {
            sentCount++;
            log(`Successfully sent email to ${recipient.email}`);
      ...
      if (sentCount === 0) {
        ...
        log(`Sending execution logs email to cemuchay@gmail.com...`);
        await sendEmailWithFallback("cemuchay@gmail.com", `[Logs] weekly-report - No recipients eligible`, logsHtml);
      } else {
    ...
    log(`[ERROR] Fatal execution failure in action "${action}": ${(error as any).message}`);
    try {
      // Attempt to send logs to developer to alert about the failure
      const logsHtml = getLogsOnlyHtml(action, logs);
      await sendEmailWithFallback("cemuchay@gmail.com", `[Logs/Error] ${action} execution failed`, logsHtml);
    } catch (sendErr) {
      console.error("Failed to send error logs to developer:", (sendErr as any).message);
    }
