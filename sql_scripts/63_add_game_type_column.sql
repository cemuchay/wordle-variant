-- 63_add_game_type_column.sql
-- Add game_type column to wordup_matches to differentiate async, live, and live-bot

ALTER TABLE public.wordup_matches
ADD COLUMN game_type VARCHAR(20) NOT NULL DEFAULT 'live';

-- Pre-fill existing rows based on is_bot_match and status patterns
UPDATE public.wordup_matches
SET game_type = 'live-bot'
WHERE is_bot_match = TRUE;

UPDATE public.wordup_matches
SET game_type = 'async'
WHERE is_bot_match = FALSE AND status = 'waiting';

-- Update the stored procedure to set game_type on new matches
