...
const LetterBox = memo(({ char, onChange, onKeyDown, inputRef, autoFocus }: {
    char: string;
    onChange: (val: string) => void;
    ...
    <input
        ...
        onKeyDown={onKeyDown}
        autoFocus={autoFocus}
        className="w-10 h-10 sm:w-11 sm:h-11 bg-black/40 border-2 border-white/20 rounded-lg text-center text-sm font-black uppercase text-white focus:border-correct/60 focus:bg-black/60 outline-none transition-all"
    />
));
...
function WordRow({ length, value, onChange, rowIndex, autoFocus }: {
    ...
    return (
        <div className="flex items-center gap-1.5">
            <span className="text-[10px] text-white/60 font-bold tabular-nums w-8 shrink-0 text-right">
                #{rowIndex + 1}
            </span>
            ...
            <span className="text-[10px] text-white/40 ml-1">{length}L</span>
        </div>
    );
}
