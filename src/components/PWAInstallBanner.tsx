...
export default function PWAInstallBanner() {
  const { showBanner, isIOS, isInstalling, handleInstall, handleDismiss } = usePWAInstall();
    ...
    <div className="fixed top-16 left-4 right-4 md:left-auto md:right-4 md:max-w-md z-[9999] animate-in fade-in slide-in-from-top-5 duration-300">
      <div className="bg-slate-900 border border-slate-800/80 rounded-2xl shadow-2xl shadow-black/50 p-4 md:p-5 backdrop-blur-xl relative overflow-hidden">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0 pr-6">
            <h4 className="text-sm font-bold text-white tracking-wide">
              {isInstalling ? 'Installing Variant...' : 'Install Variant'}
            </h4>
            <p className="text-xs text-slate-400 mt-1 leading-relaxed">
              {isInstalling 
                ? 'Variant is being added to your device. This will only take a moment...' 
        ...
        <div className="mt-4 flex flex-col gap-2.5">
          {isInstalling ? (
            <div className="w-full flex items-center justify-center gap-3 py-2.5 px-4 bg-slate-950/40 border border-slate-850 rounded-xl">
              <div className="w-4 h-4 border-2 border-slate-700 border-t-indigo-500 rounded-full animate-spin" />
              <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">Installing...</span>
            </div>
          ) : !isIOS ? (
            <button
