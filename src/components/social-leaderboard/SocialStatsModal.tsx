...
export const SocialStatsModal: React.FC<Props> = ({
  ...
  const [viewMode, setViewMode] = useState<ViewMode>(() => {
    const saved = safeSessionStorage.getItem("wordle_social_lb_view_mode") as ViewMode | null;
    // Default to 'feed'
  ...
  const handleSetViewMode = (mode: ViewMode) => {
    setViewMode(mode);
    safeSessionStorage.setItem("wordle_social_lb_view_mode", mode);
  };
                        ...
                        <div className="flex-1 bg-gray-800 rounded-sm overflow-hidden h-5">
                          <div
                            className={`${attempt === "X" ? "bg-rose-500" : "bg-emerald-500"
                              } h-full px-2 text-right flex items-center justify-end font-bold text-[10px] text-white transition-all duration-1000 min-w-6`}
