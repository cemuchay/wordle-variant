...
export function isCandidateValid(
   ...
   // Positional checks
   for (let i = 0; i < L; i++) {
      if (statuses[i] === "correct" && candidate[i] !== guessWord[i])
         return false;
      if (statuses[i] === "present" && candidate[i] === guessWord[i])
         return false;
   }
...
export async function simulateBotGame(
   ...
   currentPool = currentPool.filter((w) =>
      isCandidateValid(w, starter, statuses1),
   );
   ...
   for (let turn = 1; turn < 6; turn++) {
      ...
      if (
         ...
         turn >= 2 &&
         poolBefore.length >= 4 &&
         !isHintDisabled(uppercaseTarget, botGuesses)
      ) {
         ...
         if (hintData) {
            ...
            hintRecord = {
               ...
               letter: hintData.letter,
               row: turn + 1,
            };
         }
      }

      const optimalMove = findOptimalBotMove(
         poolBefore,
         allowedWords,
         wordLength,
         turn,
      ...
      currentPool = currentPool.filter((w) =>
         isCandidateValid(w, nextGuess, statuses),
      );
   ...
   const botWon = botGuesses[botGuesses.length - 1]?.every(
      (c: any) => c.status === "correct",
   );
...
export async function analyzeGame(
   ...
   const hintsUsed =
      !!options.hintsUsed ||
      (options.hintRecord !== null && options.hintRecord !== undefined);
   const rawHintRow =
      options.hintRecord?.row !== undefined ? options.hintRecord.row : -1;
   ...
   for (let turn = 0; turn < totalTurns; turn++) {
      ...
      const guessWord = row
         .map((cell: any) => cell.letter || "")
         .join("")
         .toUpperCase();
      const statuses: ("correct" | "present" | "absent")[] = row.map(
         (cell: any) => cell.status || "absent",
      );
      ...
      if (isSolved) {
         ...
         const elimRatio =
            poolBeforeCount > 0
               ? (poolBeforeCount - poolAfterCount) / poolBeforeCount
               : 1;
      ...
      const agreementScore = Math.max(
         10,
         Math.min(99, Math.round(moveRating * 9.5)),
      );
      ...
      if (isSolved) {
         ...
         scrutinyReason = `Highly effective guess! Eliminated ${eliminatedCount} ${eliminationPercentage.toFixed(1)}% of remaining candidates.`;
   ...
   if (hintsUsed) {
      const turnsRemainingAfterHint = isWin
         ? totalTurns - (targetTurnIndex + 1)
         : 99;
      ...
      if (moves[targetTurnIndex]) {
         ...
         if (hintQuality === "unnecessary" || hintQuality === "wasteful") {
            moves[targetTurnIndex].moveRating = Math.max(
               1.0,
               moves[targetTurnIndex].moveRating - 2.0,
            );
         } else if (hintQuality === "strategic") {
            moves[targetTurnIndex].moveRating = Math.min(
               10.0,
               moves[targetTurnIndex].moveRating + 0.5,
            );
         }
      }
   }
   ...
   const rawScore = avgMoveRating * 10;
   ...
   const botSimulation = await simulateBotGame(
      userStarterWord,
      uppercaseTarget,
   );
   ...
   const scoreDiff = Math.abs(userSkillScore - botSkillScore);
