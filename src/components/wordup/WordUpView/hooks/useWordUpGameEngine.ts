...
import {
   ...
   initialState,
   type GameType,
} from "./useWordUpGameEngine.types";
...
export function useWordUpGameEngine(props: EngineProps) {
   ...
   function clearAllT() {
      for (const k of Object.keys(T.current) as (keyof typeof T.current)[])
         clearT(k);
   }
   ...
   useEffect(() => {
      ...
      store.setIsBattlePlaying(
         state.phase === "playing" || state.phase === "reveal",
      );
   }, [state.phase]);

   useEffect(() => {
      useWordUpStore.getState().setCurrentIdx(state.currentRound);
   }, [state.currentRound]);
   useEffect(() => {
      useWordUpStore.getState().setTimeLeft(state.timeLeft);
   }, [state.timeLeft]);
   useEffect(() => {
      useWordUpStore.getState().setMaxTime(state.maxTime);
   }, [state.maxTime]);
   useEffect(() => {
      useWordUpStore.getState().setSelectedAnswer(state.selectedAnswer);
   }, [state.selectedAnswer]);
   useEffect(() => {
      useWordUpStore.getState().setRevealAnswers(state.revealAnswers);
   }, [state.revealAnswers]);
   useEffect(() => {
      useWordUpStore.getState().setOpponentStats(state.opponentStats);
   }, [state.opponentStats]);
   useEffect(() => {
      if (state.matchData)
         useWordUpStore.getState().setMatchData(state.matchData);
   }, [state.matchData]);
   useEffect(() => {
      if (state.questions.length > 0)
         useWordUpStore.getState().setQuestions(state.questions);
   }, [state.questions]);
   ...
   const handleMatchUpdate = useCallback(
      (newMatch: any) => {
         ...
         if (!cur) {
            dispatch({ type: "SET_MATCH_DATA", data: newMatch });
            return;
         }
         ...
         const meAns =
            (S.current.role === "player1"
               ? merged.p1_answered
               : merged.p2_answered) &&
            ((merged.p1_answers?.length || 0) > 0 ||
               (merged.p2_answers?.length || 0) > 0);
         const shouldReveal =
            gameType === "async"
         ...
         if (
            merged.current_question_index !== S.current.currentRound &&
            (merged.status === "active" ||
               merged.status === "countdown" ||
               merged.status === "waiting")
         ) {
            if (
               merged.current_question_index === 6 &&
               !T.current.lastRoundPopupTimeout
            ) {
         ...
         if (merged.status === "completed") {
            ...
            clearT("rematchHide");
            T.current.rematchHide = window.setTimeout(() => {
               dispatch({ type: "SHOW_REMATCH_BUTTON", show: false });
               clearT("rematchHide");
            }, 120000);
            onGameOver(merged);
         }
   ...
   const advanceRound = useCallback(
      (_mId: string, nextIdx: number) => {
         ...
         if (gameType === "async")
            upd[isP1 ? "p1_answered" : "p2_answered"] = false;
         else {
   ...
   const startQuestionRound = useCallback(
      (_match: any, index: number) => {
         ...
         T.current.roundInterval = window.setInterval(() => {
            ...
            dispatch({
               type: "TICK",
               timeLeft: parseFloat(remaining.toFixed(2)),
            });
            if (remaining <= 5.0) {
            ...
            if (remaining <= 0) {
               clearT("roundInterval");
               if (useWordUpStore.getState().selectedAnswer === null) {
         ...
         if (gameType === "live-bot" && q) {
            ...
            T.current.botTimeout = window.setTimeout(
               () => cb.current.handleMatchUpdate?.({ p2_answered: true }),
               bt * 1000,
            );
         }
      },
         ...
         if (match.game_type === "live" && match.question_started_at) {
            ...
            T.current.countdownInterval = window.setInterval(() => {
               ...
               if (c === 0) {
                  ...
                  wordupAudio.playGameStart();
                  if (
                     (role === "player2" || match.is_bot_match) &&
         ...
         try {
            ...
            if (mId.startsWith("bot-match-")) {
               ...
               try {
                  ...
                  uid =
                     session?.user?.id ||
                     localStorage.getItem("wordle_anon_id") ||
                     "guest-player";
               } catch {
                  uid =
                     localStorage.getItem("wordle_anon_id") || "guest-player";
               }
               ...
               if (isProceduralCategory(category)) {
                  ...
                  try {
                     ...
                     if (data && data.questions && data.encryption_key) {
                        ...
                        throw new Error(
                           "No questions returned from edge function",
                        );
                     }
                  } catch (e) {
                     console.error(
                        "[WordUp] Edge function for bot match failed, using local fallback:",
                        e,
                     );
               ...
               const match: any = {
                  ...
                  game_type: "live-bot",
                  bot_profile:
                     S.current.matchData?.bot_profile || getRandomBotProfile(),
                  status: "countdown",
               ...
               try {
                  await preloadMatchImages(raw);
               } catch {
                  triggerToast("Failed to load images.", 5000);
                  dispatch({ type: "RESET" });
            ...
            const mt = match.created_at
               ? new Date(match.created_at).getTime()
               : 0;
            if (
               match.status !== "completed" &&
               match.game_type !== "async" &&
               Date.now() - mt > 300000
            ) {
               await supabase
                  .from("wordup_matches")
            ...
            if (actualGameType === "live") {
               const oppId =
                  activeRole === "player1"
                     ...
                     : match.player1_id;
               if (oppId) {
                  const mp = await wordupNetworkGate
                     .enqueue("get", "load opp profile", async () => {
                        const { data, error } = await supabase
                           .from("profiles")
                     ...
                     .catch(() => null);
                  const op = await wordupNetworkGate
                     .enqueue("get", "load opp wordup profile", async () => {
                        const { data, error } = await supabase
            ...
            if (actualGameType === "live") {
               ...
               const ch = supabase
                  ...
                  .on(
                     ...
                     (p: any) => cb.current.handleMatchUpdate?.(p.new),
                  )
                  .on(
                     "broadcast",
                     { event: "player_answered" },
                     ({ payload }: any) => {
                        ...
                        if (
                           ...
                           u[
                              role === "player1" ? "p1_answered" : "p2_answered"
                           ] = true;
                           u[role === "player1" ? "p1_answers" : "p2_answers"] =
                              payload.answers;
                           u.p1_score =
                              role === "player1"
                                 ? payload.my_score
                  ...
                  .on("broadcast", { event: "rematch_request" }, () => {
                     ...
                     if (state.rematchState === "sent") {
                        ...
                        if (lm) {
                           ...
                           if (myId && oppId) {
                              if (myId < oppId)
                                 cb.current.acceptRematch?.(onRematchAccepted);
                              else {
                                 clearT("rematchFallback");
                                 T.current.rematchFallback = window.setTimeout(
                                    async () => {
                                       if (
                                          useWordUpStore.getState().view !==
                                          "gameover"
                                       )
                                          return;
                                       const { data } = await supabase
                                          ...
                                          .select("id")
                                          .or(
                                             `and(player1_id.eq.${myId},player2_id.eq.${oppId}),and(player1_id.eq.${oppId},player2_id.eq.${myId})`,
                                          )
                     ...
                     let rc = 20;
                     T.current.rematchCountdown = window.setInterval(() => {
                        rc--;
                        if (rc <= 0) {
                           clearT("rematchCountdown");
                           dispatch({
                              type: "SET_REMATCH_STATE",
                              state: "expired",
                           });
                        } else
                           dispatch({
                              type: "SET_REMATCH_COUNTDOWN",
                              count: rc,
                           });
                     }, 1000);
                  })
                  .on(
                     ...
                     ({ payload }: any) => {
                        clearT("rematchCountdown");
                        if (
                           useWordUpStore.getState().view !== "gameover" ||
                           state.rematchState === "expired"
                        )
                           return;
                     ...
                     (p: any) => {
                        if (p.new.status === "completed") {
                           ...
                           if (!store.questions?.length && p.new.encryption_key)
                              decryptMatchQuestions(p.new)
                                 .then((dec) => store.setQuestions(dec))
                                 .catch(() => {});
                           onGameOver(p.new);
                        }
                     },
            ...
            triggerToast(
               "Failed to load match questions. Aborting game.",
               5000,
            );
            const sd = S.current.matchData;
            if (
               sd?.id &&
               !sd.id.startsWith("bot-match-") &&
               sd.status !== "completed"
            ) {
               supabase
                  .from("wordup_matches")
                  ...
                  .then(({ error }) => {
                     if (error)
                        console.error(
                           "Failed to set completed on load error:",
                           error,
                        );
                  }, console.error);
            }
   ...
   const acceptRematch = useCallback(
      async (
         ...
         clearT("rematchCountdown");
         try {
            const nm = await wordupNetworkGate.enqueue(
               "post",
               "create rematch",
               async () => {
                  const { data, error } = await supabase
                     ...
                     .insert({
                        ...
                        player2_id:
                           gameType === "live-bot"
                              ...
                              : match.player2_id,
                        is_bot_match: gameType === "live-bot",
                        game_type: gameType,
                        bot_profile:
                           gameType === "live-bot"
                              ? match.bot_profile || "average"
                              : undefined,
                        status: "countdown",
            ...
            console.error("[WordUp] acceptRematch failed:", e);
   ...
   const sendRematch = useCallback(() => {
      ...
      if (gameType === "live")
         channel.current
            ?.send({ type: "broadcast", event: "rematch_request", payload: {} })
            .catch(console.error);
   }, [gameType]);
         ...
         if (gameType === "live")
            channel.current
               ?.send({
                  type: "broadcast",
                  event: "quick_chat",
                  payload: { text, senderRole: S.current.role },
               })
               .catch(console.error);
         else
            window.dispatchEvent(
               new CustomEvent("wordup-quick-chat", {
                  detail: { text, senderRole: S.current.role },
               }),
            );
      },
      [gameType],
   ...
   const purgeAndReset = useCallback(async () => {
      ...
      try {
         ...
         if (user) {
            ...
            const { data: stale } = await supabase
               .from("wordup_matches")
               .select(
                  "id, player1_id, player2_id, p1_answered, p2_answered, status",
               )
               .or(`player1_id.eq.${user.id},player2_id.eq.${user.id}`)
               .in("status", ["waiting", "countdown"]);
            if (stale)
               for (const m of stale) {
                  if (m.player1_id === user.id ? m.p1_answered : m.p2_answered)
                     await supabase
                        .from("wordup_matches")
                        .update({
                           status: "completed",
      ...
      for (const k of Object.keys(sessionStorage)) {
         if (k.startsWith("wordup_")) safeSessionStorage.removeItem(k);
      }
      for (const k of Object.keys(localStorage)) {
         if (k.startsWith("wordup_")) safeLocalStorage.removeItem(k);
      }
      dispatch({ type: "RESET" });
      triggerToast(
         "Local data purged and stale matches cleaned.",
         WORDUP_TIMEOUT.TOAST_DURATION,
      );
