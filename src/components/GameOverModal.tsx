...
export const GameOverModal: React.FC<Props> = ({
  ...
  return (
    <ModalLayout isOpen={isOpen} onClose={onClose} maxWidth="full" theme="dark">
      <div className="w-full max-w-xl mx-auto flex flex-col flex-1 h-full min-h-0 p-4 sm:p-6 overflow-y-auto text-center justify-between">
        <div>
          <div className="mb-3 mt-2 flex flex-col items-center">
            {showWord ? (
              ...
              <button
                onClick={revealWord}
                className="group flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-all cursor-pointer"
              >
                <Eye
                  size={14}
          ...
          <div className="mb-8">
            ...
            {isAuthenticated && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3 bg-gray-800 hover:bg-gray-700 active:scale-95 rounded-xl text-[10px] font-black uppercase tracking-widest text-gray-300 transition-all border border-white/5 cursor-pointer"
              >
                  ...
                  <div key={num} className="flex items-center gap-2 text-xs">
                    ...
                    <div className="flex-1 h-4 bg-gray-800/50 rounded-md">
                      <div
                        ...
                        className={`h-full flex items-center justify-end px-1 font-bold text-white transition-all duration-1000 ${
                          isCurrentDist
                            ? "bg-correct"
                            : num === "X"
                            ? "bg-red-400"
        ...
        <div className="pt-4 border-t border-gray-800">
          ...
          <button
            onClick={onClose}
            className="w-full py-3 bg-gray-800 hover:bg-gray-700 text-white rounded-xl font-bold uppercase text-xs tracking-widest transition-colors cursor-pointer"
          >
            Close
          </button>
        </div>
