...
export type WordGridPvPViewType =
   | "lobby"
   | "matchmaking"
   | "active"
   | "completed";
...
export function savePvPSnapshot(
   matchId: string | null,
...
export function loadPvPSnapshot(
   matchId: string | null,
...
export const useWordGridPvPStore = create<WordGridPvPState>((set, get) => ({
      ...
      if (playersList.length === 0) {
         playersList = [
            ...
            },
            {
               ...
               username: record.player2?.username || "Player 2",
   ...
   submitMove: async (userId, triggerToast) => {
      ...
      if (!pvpRes.success || !pvpRes.updatedState || !pvpRes.payloadToSave)
         return false;
      ...
      const safePayload = {
         ...
         current_turn: isUuid(pvpRes.payloadToSave?.current_turn)
            ? pvpRes.payloadToSave.current_turn
            : null,
      };
   ...
   exchangeTiles: async (userId, lettersToExchange, triggerToast) => {
      ...
      const activePlayer = exRes.updatedState.players?.find(
         (p) => p.id === userId,
      );
      const updatedState = {
         ...state,
      ...
      const safeExPayload = {
         ...exRes.payloadToSave,
         current_turn: isUuid(exRes.payloadToSave?.current_turn)
            ? exRes.payloadToSave.current_turn
            : null,
      };

      supabase
         .from("wordgrid_matches")
         .update(safeExPayload)
         .eq("id", state.matchId)
         .then(({ error }) => {
            if (error) {
               console.warn("[WordGridPvP] Async DB exchange error:", error);
               triggerToast?.(
                  `Cloud sync warning: ${error.message || "Failed to exchange tiles"}`,
                  4000,
               );
            }
         });
   ...
   startQueue: async (
      ...
      try {
         ...
         const { rack: p1Rack, newBag } = await drawBalancedRack(
            initialBag,
            [],
            7,
            true,
