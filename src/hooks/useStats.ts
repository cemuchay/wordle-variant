...
export const useWordleStats = (user: AppUser | null, isOpen: boolean, date: string|null) => {
  ...
  useEffect(() => {
    if (userId && date) {
      ...
      Object.keys(localStorage).forEach((key) => {
        ...
        if (key.startsWith(prefix) && key !== todayKey && key !== 'wordle-statistics') {
  ...
  const fetchCloudStats = useCallback(async () => {
    ...
    if (isMounted.current && !error && data) {
      const constructed: GameStats = JSON.parse(JSON.stringify(INITIAL_STATS));
      ...
      constructed.gamesPlayed = data.filter(s => s.status !== 'playing').length;
