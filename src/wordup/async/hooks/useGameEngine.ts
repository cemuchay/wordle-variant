...
export function useGameEngine(props: EngineProps) {
   const { gameType, matchId, role, getSyncedNow, triggerToast, onGameOver } =
      props;
   ...
   const clearT = useCallback((name: keyof typeof T.current) => {
      ...
      if (h === null) return;
      if (name === "roundInterval" || name === "countdownInterval")
         clearInterval(h);
      else clearTimeout(h);
      T.current[name] = null;
   }, []);
   ...
   useEffect(() => {
      useAsyncStore.getState().setCurrentIdx(state.currentRound);
   }, [state.currentRound]);
   useEffect(() => {
      useAsyncStore.getState().setTimeLeft(state.timeLeft);
   }, [state.timeLeft]);
   useEffect(() => {
      useAsyncStore.getState().setMaxTime(state.maxTime);
   }, [state.maxTime]);
   useEffect(() => {
      useAsyncStore.getState().setSelectedAnswer(state.selectedAnswer);
   }, [state.selectedAnswer]);
   useEffect(() => {
      useAsyncStore.getState().setRevealAnswers(state.revealAnswers);
   }, [state.revealAnswers]);
   useEffect(() => {
      useAsyncStore.getState().setOpponentStats(state.opponentStats);
   }, [state.opponentStats]);
   useEffect(() => {
      if (state.matchData)
         useAsyncStore.getState().setMatchData(state.matchData);
   }, [state.matchData]);
   useEffect(() => {
      if (state.questions.length > 0)
         useAsyncStore.getState().setQuestions(state.questions);
   }, [state.questions]);
   ...
   useEffect(() => {
      if (state.phase === "loading" || state.phase === "countdown") {
         ...
         T.current.safetyTimeout = window.setTimeout(() => {
            if (
               useAsyncStore.getState().view === "battle" ||
               useAsyncStore.getState().view === "countdown"
            )
               return;
            console.warn(
               "[WordUp] Safety timeout — async game failed to start",
            );
            triggerToast(
   ...
   const handleAnswerSelect = useCallback(
      async (choice: string) => {
         ...
         let points = 0;
         if (correct) {
            const eff = Math.max(0, elapsed - GRACE_DECAY);
            const denom = duration - GRACE_DECAY;
            points = Math.max(
               11,
   ...
   const startQuestionRound = useCallback(
      (_match: any, index: number) => {
         ...
         const duration = q
            ? getQuestionDuration(q.type)
            : QUESTION_DURATION.default;

         // Recover time left if we reloaded
         let elapsed = 0;
         const recoveredGame = safeLocalStorage.getItem(
            "wordup_async_active_game",
         );
         if (recoveredGame) {
            try {
         ...
         T.current.roundInterval = window.setInterval(() => {
            ...
            if (remainingTime <= 0) {
               clearT("roundInterval");
               if (useAsyncStore.getState().selectedAnswer === null) {
