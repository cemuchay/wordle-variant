...
import { gameEngineReducer, initialState, type GameType } from "./useGameEngine.types";
...
export function useGameEngine(props: EngineProps) {
   ...
   useEffect(() => { useLiveStore.getState().setCurrentIdx(state.currentRound); }, [state.currentRound]);
   useEffect(() => { useLiveStore.getState().setTimeLeft(state.timeLeft); }, [state.timeLeft]);
   useEffect(() => { useLiveStore.getState().setMaxTime(state.maxTime); }, [state.maxTime]);
   useEffect(() => { useLiveStore.getState().setSelectedAnswer(state.selectedAnswer); }, [state.selectedAnswer]);
   useEffect(() => { useLiveStore.getState().setRevealAnswers(state.revealAnswers); }, [state.revealAnswers]);
   useEffect(() => { useLiveStore.getState().setOpponentStats(state.opponentStats); }, [state.opponentStats]);
   useEffect(() => { if (state.matchData) useLiveStore.getState().setMatchData(state.matchData); }, [state.matchData]);
   useEffect(() => { if (state.questions.length > 0) useLiveStore.getState().setQuestions(state.questions); }, [state.questions]);
   ...
   const handleMatchUpdate = useCallback((newMatch: any) => {
      ...
      const shouldReveal = both && !S.current.revealAnswers && !G.current.isRevealing;
   ...
   cb.current.handleMatchUpdate = handleMatchUpdate;
    ...
    const advanceRound = useCallback((_mId: string, nextIdx: number) => {
       ...
       upd.p1_answered = false; upd.p2_answered = false;
       if (gameType === "live")
          ...
          if (actualGameType === "live") {
            ...
            const ch = supabase.channel(`wordup_match_${mId}`, { config: { broadcast: { self: false } } })
               ...
               .on("broadcast", { event: "rematch_accepted" }, ({ payload }: any) => {
                  ...
                  if (useLiveStore.getState().view !== "gameover" || state.rematchState === "expired") return;
