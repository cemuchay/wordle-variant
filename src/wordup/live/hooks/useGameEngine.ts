...
export function useGameEngine(props: EngineProps) {
   ...
   useEffect(() => {
      useLiveStore.getState().setOpponentStats(state.opponentStats);
   }, [state.opponentStats]);
   useEffect(() => {
      if (state.matchData)
         useLiveStore.getState().setMatchData(state.matchData);
   }, [state.matchData]);
   useEffect(() => {
      if (state.questions.length > 0)
         useLiveStore.getState().setQuestions(state.questions);
   }, [state.questions]);
   ...
   useEffect(() => {
      if (state.phase === "loading" || state.phase === "countdown") {
         ...
         T.current.safetyTimeout = window.setTimeout(() => {
            if (useLiveStore.getState().view === "battle") return;
   ...
   const startCountdown = useCallback(
      (match: any) => {
         ...
         useLiveStore.getState().setView("countdown");
      ...
      async (mId: string, activeRole: "player1" | "player2") => {
         ...
         try {
            ...
            const mt = match.created_at
               ...
               : 0;
            if (match.status !== "completed" && Date.now() - mt > 300000) {
               await supabase
                  .from("wordup_matches")
                  .update({
                     status: "completed",
               ...
               const ch = supabase
                  ...
                  .on(
                     ...
                     ({ payload }: any) => {
                        ...
                        if (
                           ...
                           const targetAnswers =
                              role === "player1" ? "p1_answers" : "p2_answers";
                           const targetScore =
                              role === "player1" ? "p1_score" : "p2_score";
                           ...
                           const newScore = newAnswers.reduce(
                              (s: number, a: any) => s + (a.points || 0),
                              0,
                           );
                           cb.current.handleMatchUpdate?.({
                              [targetAnswers]: newAnswers,
                  ...
                  .on(
                     ...
                     ({ payload }: any) => {
                        clearT("rematchCountdown");
                        if (
                           useLiveStore.getState().view !== "gameover" ||
